...
`biology_node.build_node()` returns a `DomainNode`. Measurements are currently sampled
from literature-reported summary statistics (documented ranges, not invented
precision); replace with raw morphometry / FIA data via `biology_node.ingest_fia` (and
