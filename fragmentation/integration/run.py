...
for sub in ["", "../discovery",
            ...
            "../governance/organizations", "../governance/opensource",
            "../natural-systems/mechanism", "../natural-systems/negative-controls"]:
...
def all_self_org_nodes():
    rivers = _load(os.path.join(HERE, "../natural-systems/rivers/river_node.py"), "river2c")
    neuro = _load(os.path.join(HERE, "../natural-systems/biology/neuro_node.py"), "neuro2d")
    nodes = [rivers.build_node()]                                # REAL rivers (HydroRIVERS NA)
    nodes += neuro.build_nodes()                                 # REAL biology (NeuroMorpho arbors)
    corporate = _load(os.path.join(HERE, "../governance/corporate/corporate_node.py"), "corp2b")
    organizations = _load(os.path.join(HERE, "../governance/organizations/org_node.py"), "org2e")
...
def control_nodes():
    nc = _load(os.path.join(HERE, "../natural-systems/negative-controls/real_controls.py"), "nc4real")
...
def negative_control_block(self_org, controls):
    nc = _load(os.path.join(HERE, "../natural-systems/negative-controls/controls_analysis.py"), "nca4")
...
def dgs_real_block():
    ...
    panel_path = os.path.join(HERE, "../governance/political/results/dgs_panel_real.json")
    sens_path = os.path.join(HERE, "../governance/political/results/dgs_sensitivity.json")
...
def main():
    ...
    summary = {
        ...
        "note": "MIXED inputs: REAL = rivers (HydroRIVERS NA, discovery continent), "
                ...
                "governance/corporate/results/SEALED_HOLDOUT_CORPORATE.md), negative "
