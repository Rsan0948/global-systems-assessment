...
> This page is a narrative summary. The authoritative evidence design,
> verdict, scope, and corpus counts live in
> `mi-research/docs/CLAIMS_LEDGER.md`.
...
> The MI tests a structural hypothesis; the modern era's unprecedented capital mobility
...
> descriptive and provisional.*

- **Durability gate (Safeguard J).** The recorded hindsight-calibrated
  derivation result is 17 of 19. The full modern holdout does not beat
  simple income or fragility baselines. Treat the signal as provisional
  until it passes an independent or prospective test.
- **The historical ordinality tally is not a forecast accuracy rate.**
  The 51-case tally mixes mechanical calls, abstentions, and human
  judgment. It is retained as a retrodictive audit artifact, not as
...
- **MI:** a deterministic five-pillar diagnostic with 84 modern
  hindsight-retrodictive cases, 25 ancient interpreter-scored cases, and
  67 separate out-of-sample observations. The out-of-sample evidence is
  mixed, one prospective test failed, and the sealed forecasts are pending.
- **Collectivization:** predecessor depth is strongly associated with fewer
  form changes in the current curated coding. Negotiation is an interesting
  small-group result, not a settled general rule.
- **Fragmentation:** river branching concentration passed a sealed geographic
  holdout. The broader grown-versus-designed contrast is promising but mixes
  evidence types. The universal constant and current DGS instability claim
  were not supported.
