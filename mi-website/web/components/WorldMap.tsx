...
import { MAP_W, MAP_H } from "@/lib/config";
...
export default function WorldMap({
  ...
  const fill = (f: MapFeature) => {
    if (layer === "coverage") return f.slug ? "#46637e" : "#16161f";
    return f.color;
  };
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        />
        <svg viewBox={`0 0 ${MAP_W} ${MAP_H}`} className="relative w-full" style={{ filter: "drop-shadow(0 8px 24px #00000066)" }}>
          <defs>
        ...
        {hover && (
          <div
            className="pointer-events-none absolute z-20 rounded-lg border border-border bg-surface/95 px-3 py-2 shadow-xl backdrop-blur"
            style={{
              left: Math.min(pos.x + 14, MAP_W - 40),
              top: pos.y + 14,
          ...
          >
            ...
            {hover.slug ? (
              <div className="mono mt-0.5 text-[11px]">
                <span style={{ color: hover.color }}>MI {hover.mi?.toFixed(3)} · Tier {hover.tier}</span>
      ...
      <div className="mt-3">
        {layer === "structure" ? (
          ...
          <p className="mono text-[11px] text-fg3">
            <span style={{ color: "#46637e" }}>●</span> scored ·{" "}
            <span style={{ color: "#2a2a3a" }}>●</span> not yet measured — roughly half the world is dark,
            missing the public data the engine needs.
          </p>
        )}
      </div>
    </div>
